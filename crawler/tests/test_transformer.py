...
def transform(time_filter: str):
    ...
    def lookup_location(country_name, state_name, city_name):
        ...
        def find_city(query):
          doc = city_collection.find_one(query, {"_id": 1, "country_id": 1, "state_id": 1}, collation={"locale": "en", "strength": 1})
          if not doc:
              return None
        ...
        def find_state(query):
            doc = state_collection.find_one(query, {"_id": 1, "country_id": 1}, collation={"locale": "en", "strength": 1})
            if not doc:
              return None
        ...
        def find_country(query):
            doc = country_collection.find_one(query, {"_id": 1}, collation={ "locale": "en", "strength": 1})
            if not doc:
              return None
    ...
    # MongoDB에서 데이터 가져오기
    raw_jsons = raw_collection.find(query).limit(20)
    cnt = 0
